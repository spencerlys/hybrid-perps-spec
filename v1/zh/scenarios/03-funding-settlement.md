...
- 平台 复刻相同金额记到用户账上：用户可用余额 +$1.00
- 平台 复刻金额 = HL 实际金额（严格对齐）
- 若 平台 复刻与 HL 实际存在分歧 → 触发对账告警
...
2. 平台 按 HL 公布的 funding_rate 对每个用户的 HYPERLIQUID 仓位复刻结算
3. 平台 复刻总额 = $498
4. 检测到偏差 = $2（0.4%）
