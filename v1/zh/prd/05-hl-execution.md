...
| HL 端清算 | 永远不让 HL 触发清算 | 所有清算由 XBIT 控制 |
...
XBIT 为每个用户维护独立的虚拟仓位记录（HL 端实际是合并的）：

| 字段 | 说明 |
|------|------|
| position_id | XBIT 内部仓位 ID |
| user_id | 用户 |
| symbol | 合约品种 |
| direction | 用户方向 |
| size | 持仓数量 |
...
**XBIT 不依赖自行计算的价格作为仓位最终记录，而是用 HL 返回的实际成交价更新内部数据。**
...
```
1. XBIT 按当前 mark_price 预计算 → 冻结保证金
2. 向 HL 发送开仓订单
...
```
HL 实际亏损 > XBIT 计算亏损：
  → 差额 = HL_actual_loss - XBIT_calculated_loss
  → 从风险准备金扣除差额
  → 用户按 XBIT 计算结果结算（用户无感知）
